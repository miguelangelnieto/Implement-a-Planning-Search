...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        al = self.a_levels[level-1]

        for pg_a in al:
            eff_nodes = pg_a.effnodes
            for pg_s in eff_nodes:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if (node_a1.action.effect_add == node_a2.action.effect_rem) or (node_a2.action.effect_add == node_a1.action.effect_rem):
            return True
        else:
            return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if (node_a1.action.effect_add == node_a2.action.precond_neg) or (node_a2.action.effect_add == node_a1.action.precond_neg):
            return True
        else:
            return False
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        # for each goal expression in the problem, determine the level cost, then add them together
        for goal_expr in self.problem.goal:
            for s_level in self.s_levels:
                # Goal S node
                pg_s = PgNode_s(goal_expr, True)
                # if the Goal S node is not the nodes of that level, we need to check the next one
                if pg_s not in s_level:
                    level_sum += 1
        return level_sum
